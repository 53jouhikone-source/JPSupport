#!/bin/bash
# run-jpsupport-qt6-ubuntu.sh
#
# Ubuntu 24.04ベースの、Qt6版Lazarus(ソースビルド済み)環境を起動します。
# Dockerfile.qt6.ubuntu 側で全ビルドが完了しているため、このスクリプトは
...
IMAGE_NAME="jpsupport-qt6-ubuntu"
CONTAINER_NAME="jpsupport-qt6-ubuntu"
USER_UID="$(id -u)"
USER_GID="$(id -g)"
USER_NAME="$(id -un)"

mkdir -p "${PROJECT_ROOT}/workspace-qt6"
xhost +local:docker >/dev/null 2>&1 || true

echo "[1/2] Building image ${IMAGE_NAME} (初回は時間がかかります) ..."
docker build \
    -f "${SCRIPT_DIR}/Dockerfile.qt6.ubuntu" \
...
docker run -it --rm \
    ...
    -v "${PROJECT_ROOT}/workspace-qt6:/home/${USER_NAME}/workspace:rw" \
