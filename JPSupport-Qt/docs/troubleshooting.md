...
再起動後、`半角/全角`キーを押してインジケーターアイコンが反応するか確認してください。**再起動直後は、数十秒程度は反応が不安定なことがあります。** すぐに反応しない場合、少し待ってから再度試すか、対象アプリを一度閉じて開き直してみてください(実機での検証で、再起動直後は無反応でも、少し時間を置いてから同じアプリで再度試したところ問題なく動作した、という事例を確認しています)。

これで直らない場合は、以下を順に確認してください。
