...
以下の項目を自動チェックし、問題がある場合はヒントを表示します。

- 環境変数（GTK_IM_MODULE・XMODIFIERS）の確認
- GTK2 IMモジュールの存在確認
...
Qt5・Qt6バックエンド向けの姉妹プロジェクト、[JPSupport-Qt](./JPSupport-Qt/)を公開しています。GTK2版とは異なるアプローチ（LCL本体・libQt5Pas/libQt6Pasへの直接パッチ）で、SynEditの日本語入力対応（確定処理、変換候補ウィンドウのカーソル追随、変換中の文節ハイライト表示など）を実現しています。Qt5/Qt6版のLazarusをお使いの方はそちらをご覧ください。

## 既知の制限事項

- 画面描画システム(GTK2)バックエンドのみ対応（GTK3は未対応。Qt5・Qt6は[JPSupport-Qt](./JPSupport-Qt/)を参照）
